...
window.PAGES = {
  timeOffPolicy: {
    ...
    html: '<p>We operate at a high level, which means we also need time to reset. Time off is built on trust, responsibility and impact:</p>'
      + '<ul><li>Take time off before you need it, not once you\'re already burned out.</li>'
  ...
  quarry: {
    ...
    html: '<p>The Quarry is Carrara\'s AI harness: a curated mix of tools, best practices and conventions so every Carrarian has a shared way of doing AI, and we co-build new skills together.</p>'
      ...
      + '<h4>Set up</h4>'
      + '<ul><li>Download the Claude macOS app (claude.com/download) and log in with your Carrara account (Continue with Google). Make sure you\'re on the Carrara workspace, Team plan.</li>'
      + '<li>Download Obsidian (obsidian.md) and create a synced vault. Sync needs a subscription, so use your SaaS card.</li>'
  ...
  teamRoundup: {
    ...
    html: '<p>The monthly company-wide catch-up: wins, client updates, and where the business is heading. Everyone joins.</p>'
      + '<p>Agendas and recaps are archived after each session. Your manager will make sure the invite is on your calendar; if it\'s not there by week one, ask.</p>'
      + '<div class="wk-links">'
  ...
  talentTeamMeeting: {
    ...
    html: '<p>The full talent team syncs once a week: open roles across clients, process questions, hiring experiments worth copying, and team news.</p>'
      + '<p>If you\'re on the talent side you\'ll be added to the invite in week one. Past jams are archived; day to day the conversation lives in #f-talent-general.</p>'
  },
  ...
  skuBizOps: {
    ...
    note: '',
    html: '<ul>'
      + '<li>Takes the important, ambiguous problems that don\'t sit inside any one department. Internally this is the special projects team.</li>'
  ...
  bvLogo: {
    ...
    html: '<div class="pg-brand">'
      + '<p class="body-copy">Carrara marble, and the craft of turning raw stone into art.</p>'
      + '<ul class="bv-ul">'
      + '<li><b>The wordmark.</b> Geometric letterforms with angular cuts and subtle interior curves, echoing a chisel against marble.</li>'
      + '<li><b>The blocks.</b> The raw material itself. Solid forms waiting to be shaped.</li>'
      + '<li><b>The idea.</b> Michelangelo believed the statue already existed inside the block. We see the same in foundations that need the right craft to reveal them.</li>'
      + '</ul>'
      ...
      + '<p class="body-copy">A simplified mark taken from the modular block in the logo architecture.</p>'
      + '<ul class="bv-ul">'
      + '<li><b>Use it</b> for social profile pictures, app icons, favicons, anything under 100px, and animation or loading states.</li>'
      + '<li><b>Everywhere else</b>, including presentations, documents and marketing, use the full wordmark.</li>'
      + '</ul>'
  ...
  bvTypography: {
    ...
    note: 'Source files live on Drive. Those links are marked inside.',
    html: '<div class="pg-brand">'
      + '<p class="body-copy">Two typefaces, each with a job. Portrait carries the character, Manrope carries the information.</p>'
      + '<ul class="bv-ul">'
      + '<li><b>Portrait Serif.</b> Headlines, large figures, anywhere the brand should speak confidently. Bold and Regular.</li>'
      + '<li><b>Manrope.</b> Body copy and interface text. Clean, legible, and the default for anything functional. Bold and Medium.</li>'
      + '<li><b>PT Serif.</b> The fallback when Portrait isn\'t available, for instance in a document without the font licence.</li>'
      + '</ul>'
  ...
  bvExpressions: {
    ...
    html: '<div class="pg-brand">'
      ...
      + '<p class="body-copy">A visual system for stages of work, from empty outline blocks through to fully solid ones. The same idea as revealing potential inside raw material.</p>'
      + '<p class="body-copy">Use it for process visualisation, progress indicators and infographics.</p>'
      ...
      + '<li>Generated images should feel handcrafted, not digitally smooth. Use them for proposals and presentations when photography isn\'t available.</li>'
      + '</ul>'
