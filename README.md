...
- `codex.json`: the full self-contained dataset behind the per-client pages (`#/client/<slug>`): status, project types, Carrara staffing, an about line, engagement bullets, company facts, background notes, and doc links. Compiled from the Notion Client Codex, the Quarry Brain, and internal research; all content lives on the hub with no Notion redirects. Keyed by the exact client name in `clients.json`. Snapshot, not live: refresh it when accounts or staffing change.
